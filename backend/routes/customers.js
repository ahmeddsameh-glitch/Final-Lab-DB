...
/**
 * 4.5) Set item quantity in cart (for - n + UI)
 * PUT /api/customers/:id/cart/:isbn
 * body: { qty: number }
 */
router.put('/:id/cart/:isbn', async (req, res) => {
  try {
    const id = Number(req.params.id);
    const isbn = String(req.params.isbn || '').trim();
    const qty = Number(req.body.qty);

    if (!id || !isbn || !Number.isFinite(qty) || qty < 0) {
      return res.status(400).json({ ok: false, error: 'Invalid input' });
    }
    ...
    // set exact qty (insert if not exists)
    await pool.query(
      `
      INSERT INTO cart_items (cart_id, isbn, qty)
      VALUES (?, ?, ?)
    ...
    for (const item of items) {
      ...
      await conn.query(
        `
                ...
                VALUES (?, ?, ?, ?)
                `,
        [order.insertId, item.isbn, item.qty, item.qty * item.selling_price]
      );
    }
